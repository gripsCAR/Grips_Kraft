...
namespace grips_kinematics
{
  ...
  void KinematicInterface::getJointPositions(std::vector<double> &joint_positions)
  {
     kinematic_state_->copyJointGroupPositions(joint_model_group_, joint_positions);    
  }
  
  void KinematicInterface::setJointPositions(std::vector<double>& joint_positions)
  {
     kinematic_state_->setJointGroupPositions(joint_model_group_, joint_positions);    
  }
  
  bool KinematicInterface::setEndEffectorPose(const Eigen::Affine3d &pose, unsigned int attempts, double timeout)
  {
  ...
  bool KinematicInterface::setEndEffectorPose(const geometry_msgs::Pose &pose, unsigned int attempts, double timeout)
  {
    // Here [attempts] is the number of random restart and [timeout] is the allowed time after each restart
    return kinematic_state_->setFromIK(joint_model_group_, pose, attempts, timeout);
  }
  
  bool KinematicInterface::getManipulabilityIndex(double &manipulability_index) const
  {
    return kinematics_metrics_->getManipulability(*kinematic_state_, joint_model_group_, manipulability_index);  
  }
  
  bool KinematicInterface::getManipulability(double &manipulability) const
  {
