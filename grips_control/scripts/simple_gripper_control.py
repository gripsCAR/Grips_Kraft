...
class SimpleGripper:
	joint_names = ['r_inner', 'l_inner', 'r_outer', 'l_outer', 'r_finger', 'l_finger']
	update_rate = 1000 # Hz
	ratio = 1000
	...
	def __init__(self):
		...
		rospy.Subscriber('%sr_inner/state' % self.ns, JointControllerState, self.cb_r_finger)         
		rospy.Subscriber('%sl_inner/state' % self.ns, JointControllerState, self.cb_l_finger)
		rospy.Subscriber('%sGRIP/command' % self.ns, Float64, self.cb_command)         
